 ...
 */
 ...
 intro.appendChild(el('p', null, rich('This site has **two modes**, switched at the top right. **🙂 For me** (amber) is written for the person it happened to: plain words, no tools assumed. **🛡 For responders** (cyan) is for whoever holds the alert at work. The colour tells you which one you are reading, and nothing is locked away: each mode still links to the other.')));
 ...
 frag.appendChild(pageHead(aud === 'pro' ? 'Responder mode' : 'Personal mode',
 'Breachlight',
 '🔦',
 aud === 'pro'
 ...
 ['They threatened to come to my home', '#/play/threat-at-home'],
 ['A tracker is following me', '#/play/tracker-alert'],
 ...
 if (other.length) {
 frag.appendChild(sectionHead(aud === 'pro' ? '🙂 Written for the person it happened to' : '🛡 Written for responders',
 aud === 'pro'
 ...
 */
 ...
 frag.appendChild(pageHead((p.aud === 'pro' ? 'Responder · ' : '') + (cat ? cat.title : 'Playbook'),
 ...
 frag.appendChild(sectionHead('Tell us what is wrong',
 'Corrections are welcome and they are read by a person.'));
 frag.appendChild(el('p', null, rich('Found something wrong, out of date, confusing or broken? The most useful thing you can report is **the word you searched for that found nothing**. Please do not include anything sensitive from a real incident.')));
 frag.appendChild(el('div', { class: 'btn-row' }, [
 el('a', {
 class: 'btn',
 href: 'mailto:breachlight@labidi.eu?subject=' +
 encodeURIComponent('Breachlight feedback') + '&body=' +
 encodeURIComponent('What page or search were you on?\n\n\n' +
 'What is wrong, missing or confusing?\n\n\n' +
 'Anything else?\n\n\n' +
 '---\nBreachlight v' + version),
 }, '✉ Report a mistake or a bug'),
 ]));
 frag.appendChild(el('p', { class: 'muted' }, rich('Version ' + version + '. Lives at breach.labidi.eu; built in the rami.party workshop.')));
 ...
 */
 ...
 frag.appendChild(sectionHead(aud === 'pro' ? '🙂 What the person affected notices' : '🛡 What a security team watches for',
 'Different mode, still readable, switch at the top to make it the default.'));
 const g2 = el('div', { class: 'grid wide' });
