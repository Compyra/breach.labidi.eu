...
anything is written. **Flat mode (default)**: one file where every row keeps
every column and each mail item gets its own line carrying the full record
(message, IP, device and timestamp on one row); optionally rows split into
one file per workload, never by column, with an opt-in that drops columns
...
- **No em dashes anywhere.** Use a colon for an explanation, a comma before a
  conjunction, parentheses for an aside, or a full stop for a second statement.
  Short sentences are easier to read when frightened, which is the whole point.
  Placeholders in code use `n/a` and separators use ` · `, never a dash.
...
1. `python -m http.server 8877 --bind 127.0.0.1` from the repository root
2. Structure + search:
...
3. Logscope parser and rules:
 `powershell -File logscope/run-selftest.ps1` → expect `146 / 146 passed`.
...
## Backlog
- [x] ~~The move to breach.labidi.eu~~: done 2026-08-07; see MIGRATION.md.
 rami.party keeps hash-preserving redirect stubs and a self-destructing SW.
