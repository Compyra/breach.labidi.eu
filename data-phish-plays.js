...
window.BL_PLAYS.push(
    ...
    /* ------------------------------------------------------ LOG COLLECTION */
    {
        ...
        queries: [
            ...
            { label: 'Unified Audit Log export for the affected users', lang: 'PowerShell · Exchange Online', q: 'Connect-ExchangeOnline\n$start   = (Get-Date).AddDays(-90)\n$end     = Get-Date\n$users   = @("user@contoso.com")\n# ReturnLargeSet pages by REUSING one SessionId until it returns nothing.\n# A new SessionId per loop restarts the search and returns the same rows forever.\n$session = "bl-" + (Get-Date -Format yyyyMMddHHmmss)\n$all     = @()\ndo {\n  $batch = Search-UnifiedAuditLog -StartDate $start -EndDate $end `\n             -UserIds $users -ResultSize 5000 `\n             -SessionId $session -SessionCommand ReturnLargeSet\n  if ($batch) { $all += $batch }\n} while ($batch.Count -gt 0)\n# ReturnLargeSet does not sort. Sort before you read it as a timeline.\n$all | Sort-Object CreationDate -Unique |\n  Export-Csv .\\ual-export.csv -NoTypeInformation -Encoding UTF8\n"exported $($all.Count) records"' },
        ],
    ...
    nodes: {
        ...
        'r-push': {
            ...
            steps: [
                ...
                'Move that user to a phishing-resistant method. Number matching is already enforced on Microsoft Authenticator and did not stop this, because the user was talked through reading the number out.',
            ],
